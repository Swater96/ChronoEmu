...
public:
	...
	CHRONO_INLINE sAuthLogonChallenge_C * GetChallenge() { return &m_challenge; }
	CHRONO_INLINE void SendPacket(const uint8* data, const uint16 len) { Send(data, len); }
	void OnDisconnect();
	CHRONO_INLINE time_t GetLastRecv() { return last_recv; }
	bool removedFromSet;
	CHRONO_INLINE uint32 GetAccountID() { return m_account ? m_account->AccountId : 0; }
