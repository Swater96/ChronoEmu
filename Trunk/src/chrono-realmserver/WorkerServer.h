...
public:
	...
	CHRONO_INLINE size_t GetInstanceCount() { return m_instances.size(); }
	CHRONO_INLINE void SendPacket(WorldPacket * data) { if(m_socket) m_socket->SendPacket(data); }
	CHRONO_INLINE void SendWoWPacket(Session * from, WorldPacket * data) { if(m_socket) m_socket->SendWoWPacket(from, data); }
	CHRONO_INLINE void AddInstance(Instance * pInstance) { m_instances.push_back(pInstance); }
	CHRONO_INLINE void QueuePacket(WorldPacket * data) { m_recvQueue.Push(data); }
