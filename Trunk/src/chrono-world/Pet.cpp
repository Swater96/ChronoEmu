...
void Pet::LoadFromDB(Player* owner, PlayerPet * pi)
{
	...
	// Nuke auras
	for(uint32 x = UNIT_FIELD_AURA_01; x <= UNIT_FIELD_AURA_47; x++)
		SetUInt32Value(x, 0);
}
...
void Pet::OnPushToWorld()
{
	...
	if( m_Owner && m_Owner->IsPlayer() )
		TO_PLAYER( m_Owner )->EventSummonPet( this );
...
void Pet::UpdatePetInfo(bool bSetToOffline)
{
	...
	{
		if(index >= UNIT_FIELD_AURA_01 && index <= UNIT_FIELD_AURA_47)
			ss << "0 ";
		else
...
//	double pet_sta_bonus = 0.3 * (double)m_Owner->BaseStats[STAT_STAMINA];
	...
	double pet_arm_bonus = 0.35 * (double)m_Owner->BaseResistance[SCHOOL_NORMAL];
	double pet_ap_bonus = 0.22 * (double)m_Owner->GetUInt32Value(UNIT_FIELD_RANGED_ATTACK_POWER);
	double pet_res_arcane = 0.4 * (double)m_Owner->GetResistance(SCHOOL_ARCANE);
	double pet_res_fire = 0.4 * (double)m_Owner->GetResistance(SCHOOL_FIRE);
	double pet_res_frost = 0.4 * (double)m_Owner->GetResistance(SCHOOL_FROST);
	double pet_res_holy = 0.4 * (double)m_Owner->GetResistance(SCHOOL_HOLY);
	double pet_res_nature = 0.4 * (double)m_Owner->GetResistance(SCHOOL_NATURE);
	double pet_res_shadow = 0.4 * (double)m_Owner->GetResistance(SCHOOL_SHADOW);

	//Base attributes from http://petopia.brashendeavors.net/html/art...ttributes.shtml
	static double R_pet_base_armor[60] = { 20, 21, 46, 82, 126, 180, 245, 322, 412, 518, 545, 580, 615,650, 685, 721, 756, 791, 826, 861, 897, 932, 967, 1002, 1037, 1072, 1108, 1142, 1177, 1212, 1247, 1283, 1317, 1353, 1387, 1494, 1607, 1724, 1849, 1980, 2117, 2262, 2414, 2574, 2742, 2798, 2853,2907, 2963, 3018, 3072, 3128, 3183, 3237, 3292, 3348, 3402, 3457, 3512, 3814 };
	static double R_pet_base_hp[60] = { 42, 55, 71, 86, 102, 120, 137, 156, 176, 198, 222, 247, 273, 300, 328, 356, 386, 417, 449, 484, 521, 562, 605, 651, 699, 750, 800, 853, 905, 955, 1006, 1057, 1110, 1163, 1220, 1277, 1336, 1395, 1459, 1524, 1585, 1651, 1716, 1782, 1848, 1919, 1990, 2062, 2138, 2215, 2292, 2371, 2453, 2533, 2614, 2699, 2784, 2871, 2961, 3052 };
	static double R_pet_base_ap[60] = { 24, 26, 28, 30, 32, 34, 36, 38, 40, 42, 44, 46, 48, 50, 54, 56, 60, 64, 68, 70, 74, 78, 80, 84, 86, 90, 92, 96, 100, 102, 106, 108, 112, 114, 118, 120, 124, 128, 132, 136, 142, 152, 162, 174, 184, 188, 192, 196, 200, 206, 210, 214, 218, 224, 228, 234, 238, 242, 248, 252 };
	static double R_pet_base_stamina[60] = { 22, 23, 24, 25, 25, 26, 27, 28, 29, 30, 34, 38, 42, 46, 51, 55, 59, 63, 67, 72, 76, 80, 84, 88, 94, 98, 102, 106, 110, 115, 120, 124, 128, 132, 138, 142, 146, 151, 155, 160, 165, 169, 174, 178, 184, 188, 193, 197, 202, 207, 212, 216, 221, 226, 231, 236, 241, 245, 250, 256 };
	...
	BaseResistance[SCHOOL_NORMAL] = float2int32(pet_armor);
	BaseResistance[SCHOOL_ARCANE] = float2int32(pet_res_arcane);
	BaseResistance[SCHOOL_FIRE] = float2int32(pet_res_fire);
	BaseResistance[SCHOOL_FROST] = float2int32(pet_res_frost);
	BaseResistance[SCHOOL_HOLY] = float2int32(pet_res_holy);
	BaseResistance[SCHOOL_NATURE] = float2int32(pet_res_nature);
	BaseResistance[SCHOOL_SHADOW] = float2int32(pet_res_shadow);
