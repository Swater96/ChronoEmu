	...
	for(itr = _respawnObjects.begin(); itr != _respawnObjects.end(); ++itr)
	{
		...
		case TYPEID_UNIT: {
				if( !(*itr)->IsPet() )
				{
					...
					static_cast< Creature* >( *itr )->m_respawnCell=nullptr;
					delete static_cast< Creature* >( *itr );
				}
			}break;

		case TYPEID_GAMEOBJECT: {
			static_cast< GameObject* >( *itr )->m_respawnCell=nullptr;
...
/*				if((*i)->respawnNpcLink && pInstance->m_killedNpcs.find((*i)->respawnNpcLink) != pInstance->m_killedNpcs.end())
	...
	if(sp->GOSpawns.size())//got GOs
	{
		for(GOSpawnList::iterator i=sp->GOSpawns.begin();i!=sp->GOSpawns.end();i++)
		{
			...
			if(go->Load(*i))
			{
				//uint32 state = go->GetByte(GAMEOBJECT_STATE, GAMEOBJECT_BYTES_STATE);

				// FIXME - burlex
				/*
				if(pInstance && pInstance->FindObject((*i)->stateNpcLink))
				{
					go->SetByte(GAMEOBJECT_STATE,GAMEOBJECT_BYTES_STATE, (state ? 0 : 1));
				}*/			   
