...
public:
	...
	CHRONO_INLINE uint32 GetXP(void) { return m_PetXP; }
	...
	CHRONO_INLINE void SetPetAction(uint32 act) { m_Action = act; }
	CHRONO_INLINE uint32 GetPetAction(void) { return m_Action; }

	CHRONO_INLINE void SetPetState(uint32 state) { m_State = state; }
	CHRONO_INLINE uint32 GetPetState(void) { return m_State; }

	CHRONO_INLINE void SetPetDiet(uint32 diet) { m_Diet = diet; }
	CHRONO_INLINE void SetPetDiet() { m_Diet = myFamily->petdietflags; }

	CHRONO_INLINE uint32 GetPetDiet(void) { return m_Diet; }
	
	CHRONO_INLINE AI_Spell* GetAISpellForSpellId(uint32 spellid)
	{
	...
	CHRONO_INLINE Player* GetPetOwner() { return m_Owner; }
	CHRONO_INLINE void ClearPetOwner() { m_Owner = 0; }
	...
	CHRONO_INLINE void RemoveSpell(uint32 SpellID)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp) RemoveSpell(sp);
	}
	CHRONO_INLINE void SetSpellState(uint32 SpellID, uint16 State)
	{
		SpellEntry * sp = dbcSpell.LookupEntry(SpellID);
		if(sp) SetSpellState(sp, State);
	}
	CHRONO_INLINE uint16 GetSpellState(uint32 SpellID)
	{
		if(SpellID == 0)
	...
	CHRONO_INLINE PetSpellMap* GetSpells() { return &mSpells; }
	CHRONO_INLINE bool IsSummon() { return Summon; }

	void __fastcall SetAutoCastSpell(AI_Spell * sp);
	void Rename(string NewName);
	CHRONO_INLINE string& GetName() { return m_name; }
