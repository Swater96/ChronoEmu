...
void Creature::RegenerateMana(bool isinterrupted)
{
	...
	if(cur>=mm)return;
	float amt=(getLevel()+10)*PctPowerRegenModifier[POWER_TYPE_MANA];
...
// Custom Setups for Instances & Raids
uint32 Creature::GetProtoItemDisplayId(uint8 slot)
{
	switch (slot)
	{
		case 0: return GetProto()->Item1SlotDisplay;
		case 1: return GetProto()->Item2SlotDisplay;
		case 2: return GetProto()->Item3SlotDisplay;
	}

	return 0;
}

bool Creature::ChangeEntry(uint32 Entry)
{
	creature_info = CreatureNameStorage.LookupEntry(Entry);
	proto = CreatureProtoStorage.LookupEntry(Entry);
	SetCreatureEntry(Entry);
    SetUInt32Value(UNIT_FIELD_BYTES_2, 0x10 );
	SetUInt32Value(UNIT_NPC_FLAGS, proto->NPCFLags);
	SetUInt32Value(UNIT_FIELD_DISPLAYID, creature_info->Male_DisplayID);
	SetUInt32Value(UNIT_FIELD_FLAGS, creature_info->Flags1);
	SetUInt32Value(UNIT_FIELD_HEALTH, proto->MaxHealth);
	SetUInt32Value(UNIT_FIELD_MAXHEALTH, proto->MaxHealth);
	SetFloatValue(UNIT_FIELD_MINDAMAGE, proto->MinDamage);
	SetFloatValue(UNIT_FIELD_MAXDAMAGE, proto->MaxDamage);
	SetUInt32Value(UNIT_FIELD_RANGEDATTACKTIME,proto->RangedAttackTime);
	SetFloatValue(UNIT_FIELD_MINRANGEDDAMAGE,proto->RangedMinDamage);
	...
	SetName(creature_info->Name);
	SetFloatValue(OBJECT_FIELD_SCALE_X, proto->Scale);
	
	for(uint32 i = 0; i < 7; ++i)
		SetUInt32Value(UNIT_FIELD_RESISTANCES+i,proto->Resistances[i]);
	for(uint32 i = 0; i < 5; ++i)
		BaseStats[i]=GetUInt32Value(UNIT_FIELD_STAT0+i);
