	...
	if(objA->IsPet())
	{
		if(objB->IsPlayer())
			if(
				...
				static_cast<Pet *>(objA)->GetPetOwner()->DuelingWith == TO_PLAYER(objB) && 
	...
	if(objB->IsPet())
	{
		if(objA->IsPlayer())
			if(
				...
				static_cast<Pet *>(objB)->GetPetOwner()->DuelingWith == TO_PLAYER(objA) && 
		...
		if(static_cast<Creature *>(objA)->IsTotem())
		{
			if(objB->IsPlayer())
				if( static_cast<Creature *>(objA)->GetTotemOwner() &&
					static_cast<Creature *>(objA)->GetTotemOwner()->DuelingWith == TO_PLAYER(objB) && 
		...
		if(static_cast<Creature *>(objB)->IsTotem())
		{
			if(objA->IsPlayer())
				if( static_cast<Creature *>(objB)->GetTotemOwner() &&
					static_cast<Creature *>(objB)->GetTotemOwner()->DuelingWith == TO_PLAYER(objA) && 
