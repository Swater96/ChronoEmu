...
#define GET_TYPE_FROM_GUID(x) (Chrono::Util::GUID_HIPART((x)) & HIGHGUID_TYPE_MASK)
#define GET_LOWGUID_PART(x) (Chrono::Util::GUID_LOPART((x)) & LOWGUID_ENTRY_MASK)
...
public:
  ...
  //! True if object exists in world
	...
	CHRONO_INLINE const uint64& GetGUID() const { return *((uint64*)m_uint32Values); }
#else
	CHRONO_INLINE const uint64 GetGUID() const { return GetUInt64Value(0); }
#endif

	CHRONO_INLINE const WoWGuid& GetNewGUID() const { return m_wowGuid; }
	CHRONO_INLINE uint32 GetEntry(){return m_uint32Values[3];}
	
	CHRONO_INLINE const uint32 GetEntryFromGUID() const
	{
/*		uint64 entry = *(uint64*)m_uint32Values;
	...
	CHRONO_INLINE const uint32 GetTypeFromGUID() const { return (m_uint32Values[1] & HIGHGUID_TYPE_MASK); }
	CHRONO_INLINE const uint32 GetUIdFromGUID() const { return (m_uint32Values[0] & LOWGUID_ENTRY_MASK); }
	CHRONO_INLINE const uint32 GetLowGUID() const { return (m_uint32Values[0]); }

	// type
	CHRONO_INLINE const uint8& GetTypeId() const { return m_objectTypeId; }
	CHRONO_INLINE bool IsUnit()	{ return ( m_objectTypeId == TYPEID_UNIT || m_objectTypeId == TYPEID_PLAYER ); }
	CHRONO_INLINE bool IsPlayer() { return m_objectTypeId == TYPEID_PLAYER; }
	CHRONO_INLINE bool IsCreature() { return m_objectTypeId == TYPEID_UNIT; }
	bool IsPet();
	...
	CHRONO_INLINE const float& GetPositionX( ) const { return m_position.x; }
	CHRONO_INLINE const float& GetPositionY( ) const { return m_position.y; }
	CHRONO_INLINE const float& GetPositionZ( ) const { return m_position.z; }
	CHRONO_INLINE const float& GetOrientation( ) const { return m_position.o; }
	CHRONO_INLINE void SetOrientation( float &o ) { m_position.o = o; }

	CHRONO_INLINE const float& GetSpawnX( ) const { return m_spawnLocation.x; }
	CHRONO_INLINE const float& GetSpawnY( ) const { return m_spawnLocation.y; }
	CHRONO_INLINE const float& GetSpawnZ( ) const { return m_spawnLocation.z; }
	CHRONO_INLINE const float& GetSpawnO( ) const { return m_spawnLocation.o; }

	CHRONO_INLINE const LocationVector & GetPosition() { return m_position; }
	CHRONO_INLINE LocationVector & GetPositionNC() { return m_position; }
	CHRONO_INLINE LocationVector * GetPositionV() { return &m_position; }
	...
	//! Only for MapMgr use
	CHRONO_INLINE MapCell* GetMapCell() const { return m_mapCell; }
	//! Only for MapMgr use
	CHRONO_INLINE void SetMapCell(MapCell* cell) { m_mapCell = cell; }
	//! Only for MapMgr use
	CHRONO_INLINE MapMgr* GetMapMgr() const { return m_mapMgr; }

	CHRONO_INLINE void SetMapId(uint32 newMap) { m_mapId = newMap; }
	void SetZoneId(uint32 newZone);

	CHRONO_INLINE const uint32 GetMapId( ) const { return m_mapId; }
	CHRONO_INLINE const uint32& GetZoneId( ) const { return m_zoneId; }

	//! Get uint32 property
	CHRONO_INLINE const uint32& GetUInt32Value( uint32 index ) const
	{
...
#else
	CHRONO_INLINE const uint64& GetUInt64Value( uint32 index ) const
#endif
	{
	...
	//! Get float property
	CHRONO_INLINE const float& GetFloatValue( uint32 index ) const
	{
		ASSERT( index < m_valuesCount );
	...
	CHRONO_INLINE uint8 GetByte(uint32 i,uint32 i1)
	{
	...
	CHRONO_INLINE void SetNewGuid(uint32 Guid)
	{
	...
	CHRONO_INLINE bool HasFlag( const uint32 index, uint32 flag ) const { return (m_uint32Values[ index ] & flag) != 0;	}
	...
	CHRONO_INLINE const float GetDistanceSq(Object* obj)
	{
		if(obj->GetMapId() != m_mapId) return 40000.0f; //enough for out of range
		return m_position.DistanceSq(obj->GetPosition());
	}

	CHRONO_INLINE float GetDistanceSq(LocationVector & comp)
	{
		return comp.DistanceSq(m_position);
	}

	CHRONO_INLINE float CalcDistance(LocationVector & comp)
	{
		return comp.Distance(m_position);
	}

	CHRONO_INLINE const float GetDistanceSq(float x, float y, float z)
	{
		return m_position.DistanceSq(x, y, z);
	}

	CHRONO_INLINE const float GetDistance2dSq( Object* obj )
	{
	...
	CHRONO_INLINE bool IsInRangeSet( Object* pObj )
	{
	...
	CHRONO_INLINE bool HasInRangeObjects()
	{
	...
	CHRONO_INLINE size_t GetInRangeCount() { return m_objectsInRange.size(); }
	CHRONO_INLINE size_t GetInRangePlayersCount() { return m_inRangePlayers.size();}
	CHRONO_INLINE InRangeSet::iterator GetInRangeSetBegin() { return m_objectsInRange.begin(); }
	CHRONO_INLINE InRangeSet::iterator GetInRangeSetEnd() { return m_objectsInRange.end(); }
	CHRONO_INLINE InRangeSet::iterator FindInRangeSet(Object * obj) { return m_objectsInRange.find(obj); }
	...
	CHRONO_INLINE bool RemoveIfInRange( Object * obj )
	{
	...
	CHRONO_INLINE void AddInRangePlayer( Object * obj )
	{
		m_inRangePlayers.insert( reinterpret_cast< Player* >( obj ) );
	}

	CHRONO_INLINE void RemoveInRangePlayer( Object * obj )
	{
		m_inRangePlayers.erase( reinterpret_cast< Player* >( obj ) );
	}
	...
	CHRONO_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetBegin() { return m_oppFactsInRange.begin(); }
	CHRONO_INLINE std::set<Object*>::iterator GetInRangeOppFactsSetEnd() { return m_oppFactsInRange.end(); }
	CHRONO_INLINE std::set<Player*>::iterator GetInRangePlayerSetBegin() { return m_inRangePlayers.begin(); }
	CHRONO_INLINE std::set<Player*>::iterator GetInRangePlayerSetEnd() { return m_inRangePlayers.end(); }
	CHRONO_INLINE std::set<Player*> * GetInRangePlayerSet() { return &m_inRangePlayers; };

	void __fastcall SendMessageToSet(WorldPacket *data, bool self,bool myteam_only=false);
	CHRONO_INLINE void SendMessageToSet(StackPacket * data, bool self) { OutPacketToSet(data->GetOpcode(), (uint16)data->GetSize(), data->GetBufferPointer(), self); }
	...
	CHRONO_INLINE uint16 GetValuesCount() const { return m_valuesCount; }
	...
	float m_last_move_Speed; //needed for speedhackchecks can't use m_lastRunSpeed for it cause that updates to fast
	...
	CHRONO_INLINE void SetInstanceID(int32 instance) { m_instanceId = instance; }
	CHRONO_INLINE int32 GetInstanceID() { return m_instanceId; }
	...
	bool m_inQueue;
	CHRONO_INLINE void SetMapMgr(MapMgr * mgr) { m_mapMgr = mgr; }
	...
	CHRONO_INLINE size_t GetInRangeOppFactCount() { return m_oppFactsInRange.size(); }
