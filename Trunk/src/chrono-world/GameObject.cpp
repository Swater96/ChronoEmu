...
{
	sEventMgr.RemoveEvents(TO_GAMEOBJECT(this));
 	if(m_ritualmembers)
 		delete[] m_ritualmembers;
}
...
uint8 GameObject::GetState(uint8 state)
{
	//We need to fix this, as We need it to get GAMEOBJECT_STATE, 
	//but what is it being set from?
	return GetByte(GAMEOBJECT_STATE, state);
}
