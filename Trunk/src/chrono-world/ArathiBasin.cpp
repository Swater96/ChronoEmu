...
static int resHonorTable[7] = { 0,  0,  4,  7,  11, 19, 20};
static int winHonorTable[7] = { 0,  0,  4,  7,  11, 19, 20};
...
void ArathiBasin::SpawnControlPoint(uint32 Id, uint32 Type)
{
	...
	gi_aura = gi->sound3 ? GameObjectNameStorage.LookupEntry(gi->sound3) : nullptr;
...
void ArathiBasin::OnCreate()
{
	...
	SetWorldState(WORLDSTATE_AB_SHOW_BACKSMITH_ICON, 			 0x01);
	SetWorldState(WORLDSTATE_AB_SHOW_FARM_ICON, 				  0x01);
	SetWorldState(WORLDSTATE_AB_SHOW_LUMBERMILL_ICON, 			0x01);
	SetWorldState(WORLDSTATE_AB_SHOW_GOLDMINE_ICON, 			 0x01);
	SetWorldState(WORLDSTATE_AB_SHOW_STABLE_ICON, 			0x01);

	// LumberMill
	SetWorldState(WORLDSTATE_AB_CAPTURING_LUMBERMILL_HORDE, 	   0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURING_LUMBERMILL_ALLIANCE, 	0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_LUMBERMILL_HORDE, 		0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_LUMBERMILL_ALLIANCE, 	 0x00);

	// GoldMine
	SetWorldState(WORLDSTATE_AB_CAPTURING_GOLDMINE_HORDE, 		 0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURING_GOLDMINE_ALLIANCE, 	  0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_GOLDMINE_HORDE, 		  0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_GOLDMINE_ALLIANCE, 	   0x00);

	// BlackSmith
	SetWorldState(WORLDSTATE_AB_CAPTURING_BLACKSMITH_HORDE, 	   0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURING_BLACKSMITH_ALLIANCE, 	0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_BLACKSMITH_HORDE, 		0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_BLACKSMITH_ALLIANCE, 	 0x00);

	SetWorldState(WORLDSTATE_AB_MAX_SCORE, 						RESOURCES_WINVAL);
	SetWorldState(WORLDSTATE_AB_ALLIANCE_CAPTUREBASE, 			 0x00);
	SetWorldState(WORLDSTATE_AB_HORDE_CAPTUREBASE, 				0x00);
	SetWorldState(WORLDSTATE_AB_HORDE_RESOURCES, 				  0x00);
	SetWorldState(WORLDSTATE_AB_ALLIANCE_RESOURCES, 			   0x00);

	// Farm
	SetWorldState(WORLDSTATE_AB_CAPTURING_FARM_ALLIANCE, 		 0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURING_FARM_HORDE, 			 0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_FARM_HORDE, 			  0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_FARM_ALLIANCE, 		   0x00);

	// Stables
	SetWorldState(WORLDSTATE_AB_CAPTURING_STABLES_HORDE, 		  0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURING_STABLES_ALLIANCE, 	   0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_STABLES_HORDE, 		   0x00);
	SetWorldState(WORLDSTATE_AB_CAPTURED_STABLES_ALLIANCE, 		0x00);
}
...
ArathiBasin::~ArathiBasin()
{
	...
	for(uint32 i = 0; i < AB_NUM_CONTROL_POINTS; ++i)
	{
		...
		{
			m_buffs[i]->m_battleground = nullptr;
		...
		{
			m_controlPoints[i]->m_battleground = nullptr;
		...
		{
			m_controlPointAuras[i]->m_battleground = nullptr;
...
void ArathiBasin::EventUpdateResources(uint32 Team)
{
	...
	if((current_resources - m_lastHonorGainResources[Team]) >= resourcesToGainBH)
	{
		for(set<Player*>::iterator itr = m_players[Team].begin(); itr != m_players[Team].end(); ++itr)
		{
			...
			(*itr)->AddHonorCP(resHonorTable[m_lgroup], HONORABLE, 0, 0);
		}
	}
		...
		for(uint32 i = 0; i < 2; ++i)
		{
			for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
			{
				...
				else
				{
					...
					(*itr)->AddHonorCP(winHonorTable[m_lgroup], HONORABLE, 0, 0);
...
void ArathiBasin::HookOnAreaTrigger(Player * plr, uint32 id)
{
	...
	if(m_buffs[x] && m_buffs[x]->IsInWorld())
	{
		...
		if(sp)
		{
			Spell * pSpell = new Spell(plr, sp, true, nullptr);
