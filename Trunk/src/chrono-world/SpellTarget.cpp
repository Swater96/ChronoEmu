...
pSpellTarget SpellTargetHandler[TOTAL_SPELL_TARGET] = 
{
	...
	&Spell::SpellTargetAreaOfEffect,					// 76
...
void Spell::SpellTargetSingleTargetEnemy(uint32 i, uint32 j)
{
	...
	if(m_spellInfo->EffectChainTarget[i])
	{
		...
		/* warning high workaround potentional */

		if(m_spellInfo->NameHash == SPELL_HASH_CLEAVE)
		{
			float NearestRange = 0;
			float NearestGUID = 0;
			for( itr = m_caster->GetInRangeSetBegin(); itr != m_caster->GetInRangeSetEnd(); itr++ )
			{
				...
				{
					if(NearestRange > m_caster->GetDistance2dSq((*itr)))
					{
						NearestRange = m_caster->GetDistance2dSq((*itr));
						NearestGUID = (*itr)->GetGUID();
					}
				}
			}
			Player* nTarget = objmgr.GetPlayer(NearestGUID);
			if(nTarget)
			{
				did_hit_result = DidHit(i,TO_UNIT(nTarget));
				if(did_hit_result==SPELL_DID_HIT_SUCCESS)
					SafeAddTarget(tmpMap, nTarget->GetGUID());
				else
					SafeAddModeratedTarget(nTarget->GetGUID(), did_hit_result);
			}
		}		
...
bool Spell::TargetsHostile(uint32 i)
{
	switch (m_spellInfo->EffectImplicitTargetB[i])
	{
	case 0: //no implicittarget B, assume targets hostile (rain of fire, blizzard for example)
	case TARGET_HOSTILE:
	case TARGET_HOSTILE2:
		return true;
...
bool Spell::TargetsFriendly(uint32 i)
{
	switch (m_spellInfo->EffectImplicitTargetB[i])
	{
