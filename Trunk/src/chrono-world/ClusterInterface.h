...
public:
	...
	CHRONO_INLINE WorldSession * GetSession(uint32 sid) { return _sessions[sid]; }
	...
	CHRONO_INLINE void QueuePacket(WorldPacket * pck) { _pckQueue.Push(pck); }

	void Update();
	void DestroySession(uint32 sid);

	CHRONO_INLINE void SendPacket(WorldPacket * data) { if(_clientSocket) _clientSocket->SendPacket(data); }
	CHRONO_INLINE void SetSocket(WSClient * s) { _clientSocket = s; }
