...
enum SpellCastTargetFlags
{
	...
	TARGET_FLAG_SELF2				= 0x0100, // hmm?
    TARGET_FLAG_PVP_CORPSE          = 0x0200,
    TARGET_FLAG_MASS_REZ            = 0x0400, //BG Rez?
...
enum procFlags
{
	PROC_NULL					= 0x0,			// Proc on Nothing?
	PROC_ON_MELEE_HIT			= 0x1,			// Proc on you melee hitting target
	PROC_TARGET_MELEE_HIT		= 0x2,			// Proc on target melee hitting you
	PROC_ON_KILL				= 0x4,			// Proc on Kills that give experience and/or Honor
	PROC_ON_WAT					= 0x8,			// Proc on No Fucking Clue
	PROC_ON_DODGE				= 0x10,			// Proc on Dodge
	PROC_ON_PARRY				= 0x20,			// Proc on Parry
	PROC_ON_BLOCK				= 0x40,			// Proc on Block
	PROC_ON_TOUCH				= 0x80,			// Proc on No Fucking Clue
	PROC_TARGET_LOW_HEALTH		= 0x100,		// Proc on Target having low health
	PROC_SELF_LOW_HEALTH		= 0x200,		// Proc on you having low health
	PROC_TARGET_RANGED_HIT		= 0x400,		// Proc on target range hitting you
	PROC_PERIODIC_TICK			= 0x800,		// Proc on Periodic Ticks
	PROC_ON_MELEE_CRIT 			= 0x1000,		// Proc on you melee critting target
	PROC_TARGET_MELEE_CRIT		= 0x2000,		// Proc on target melee critting you
	PROC_ON_CAST_SPELL			= 0x4000,		// Proc on casting a spell
	PROC_ON_TAKE_DAMAGE			= 0x8000,		// Proc on taking damage
	PROC_ON_SPELL_CRIT			= 0x10000,		// Proc on Spell criticals
	PROC_ON_SPELL_HIT			= 0x20000,		// Proc on Spell Hit
	PROC_TARGET_SPELL_CRIT		= 0x40000,		// Proc on target spell critting you
	PROC_ON_RANGED_HIT			= 0x80000,		// Proc on you ranged hitting target
	PROC_TARGET_SPELL_HIT		= 0x100000, 	// Proc on Target spell hitting you
	PROC_ON_TRAP_TRIGGER		= 0x200000,		// Proc on activating a trap
	PROC_ON_RANGED_CRIT			= 0x400000,		// Proc on you ranged critting target
	PROC_TARGET_RANGED_CRIT		= 0x800000,		// Proc on target range critting you
	PROC_ON_RESIST_SPELL		= 0x1000000,	// Proc on you resisting a enemy spell
	PROC_TARGET_RESIST_SPELL	= 0x2000000,	// Proc on target resisting your spell
	PROC_TARGET_DODGE_OR_PARRY	= 0x4000000,	// Proc on Target dodging and/or Parrying your attack
	PROC_ON_HEAL				= 0x8000000,	// Proc on heal
	PROC_ON_HEAL_CRIT			= 0x10000000,	// Proc on critical heal
	PROC_ON_HEALING				= 0x20000000,	// This is getting ridiculous
	PROC_TARGET_BLOCKED			= 0x40000000,	// Proc on target blocking your attack
	PROC_ON_MISS				= 0x80000000,	// Proc on missing melee attack
	
	//CPROC_ON_HOSTILE_ACTION	= PROC_TARGET_RANGED_CRIT | PROC_TARGET_SPELL_HIT | PROC_TARGET_SPELL_CRIT |
								  //PROC_ON_TAKE_DAMAGE | PROC_TARGET_MELEE_CRIT | PROC_TARGET_MELEE_HIT |
								  //PROC_TARGET_RANGED_HIT,
};

enum customProcFlags
{
	CPROC_ON_HOSTILE_ACTION		 	 = 0x1,
	CPROC_ON_SPELL_LAND              = 0x400,
	CPROC_PRE_DISPEL				 = 0x4000,	//Custom Proc for Pre-Dispel
	CPROC_CAST_SPECIFIC				 = 0x10000,	//Custom Proc for Specific Spells
	CPROC_TARGET_DIE				 = 0x80000,	//Custom Proc for Target Death
	CPROC_AUTO_SHOT					 = 0x400000,	//Custom Proc for Auto shot
	CPROC_ABSORB					 = 0x800000, //Custom Proc for Absorb
	CPROC_ON_DODGE			 		 = 0x2000000,
	CPROC_ON_DIE                     = 0x4000000,	//Custom Proc for Death
	CPROC_REMOVEONUSE				 = 0x8000000,	//Custom Proc for Bombs and Grenades
	CPROC_MISC						 = 0x10000000,	//Custom Proc for Misc Shit
	CPROC_ON_BLOCK					 = 0x20000000,
	CPROC_ON_SELF               	 = 0x80000000,	//Custom Proc for self casts
};
...
{
	ATTRIBUTES_NULL								= 0x0,
	ATTRIBUTES_UNK2								= 0x1,			// Ranged Spells
	ATTRIBUTES_UNK3								= 0x2, 			// related to ranged??
	ATTRIBUTE_ON_NEXT_ATTACK					= 0x4,			// Next Swing
	ATTRIBUTES_UNUSED0							= 0x8,
	ATTRIBUTES_UNK6								= 0x10,			
	ATTRIBUTES_UNK7								= 0x20, 		// Tradeskill recipies
	ATTRIBUTES_PASSIVE							= 0x40,			// Passive Spell
	ATTRIBUTES_NO_VISUAL_AURA					= 0x80,			// Invisible Aura
	...
	ATTRIBUTES_UNUSED1							= 0x1000,		// only usable during day
	ATTRIBUTES_UNUSED2							= 0x2000,		// Only usable at night
	ATTRIBUTES_UNUSED3							= 0x4000,		// Only useable indoors
	ATTRIBUTES_ONLY_OUTDOORS					= 0x8000,		// Only useable outdoors
	ATTRIBUTES_UNK								= 0x10000,		// Can't use in shapeshift
	ATTRIBUTES_REQ_STEALTH						= 0x20000,		// Must be in stealth
	ATTRIBUTES_UNK20							= 0x40000,		// it's not : must be behind
	ATTRIBUTES_UNK21							= 0x80000,		// Damage depends on casters level
	...
	ATTRIBUTES_UNK25							= 0x800000,		// Useable while dead
	...
	ATTRIBUTES_UNK29							= 0x8000000,	// Can cast while sitting
...
enum AttributesEx
{
	...
	ATTRIBUTESEX_DRAIN_WHOLE_MANA             = 0x2, 	   // Use All Power (Lay on Hands?)
	ATTRIBUTESEX_UNK4                         = 0x4, 	   // Channeling
	ATTRIBUTESEX_UNK5                         = 0x8, 	   // 
	ATTRIBUTESEX_UNK6                         = 0x10, 	   // stealth effects but Rockbiter wtf 0_0
	ATTRIBUTESEX_NOT_BREAK_STEALTH            = 0x20,      // Not break stealth
	ATTRIBUTESEX_UNK8						  = 0x40,      // Channeling
	ATTRIBUTESEX_UNK9                         = 0x80,
	ATTRIBUTESEX_UNK10                        = 0x100,	   // Target cannot be in combat
	ATTRIBUTESEX_UNK11                        = 0x200,
	ATTRIBUTESEX_UNK12                        = 0x400,	   // Does not generate threat
	...
	ATTRIBUTESEX_UNK16                        = 0x4000,    // unaffected by school immunity
	...
	ATTRIBUTESEX_UNK20                        = 0x40000,   // related to stun
...
enum Flags3
{
	...
	FLAGS3_UNK21              	= 0x80000, 	  // Does not need shapeshift?
	FLAGS3_REQ_BEHIND_TARGET  	= 0x100000,   // wrong
	FLAGS3_UNK23              	= 0x200000,   //
	FLAGS3_UNK24              	= 0x400000,	  //
	FLAGS3_UNK25              	= 0x800000,	  // Only Mage Arcane Concentration has this flag
	FLAGS3_UNK26              	= 0x1000000,  // 
	FLAGS3_UNK27              	= 0x2000000,  //
	FLAGS3_UNK28              	= 0x4000000,  // Unaffected by School Immunities
	...
	FLAGS3_UNK31             	= 0x20000000, // Spells Can't Crit?
	FLAGS3_UNK32              	= 0x40000000, //
	FLAGS3_UNK33			  	= 0x80000000, // Food & Drink Buffs (Well Fed)
};

enum Flags4
{
	FLAGS4_UNK							= 0x10,			// Druid Rebirth Only?
	FLAGS4_PLAYER_MAIN_HAND				= 0x400,		// Requires main hand weapon
	FLAGS4_BG_ONLY                      = 0x800,		// Only castable in battlegrounds
	FLAGS4_DEAD_SPELL					= 0x1000,		// Only castable on dead targets
	FLAGS4_PLAYER_HONORLESS				= 0x4000,		// Honorless Target only, Possibly No honor given?
	FLAGS4_PLAYER_RANGED_SPELLS         = 0x8000,		// Autoshoot, Shoot, Throw, etc
	FLAGS4_DEATH_PERSISTANT			    = 0x100000,		// Buffs that last through death
	FLAGS4_PLAYER_RANGED_WAND           = 0x400000,		// Wand is required
	FLAGS4_PLAYER_OFF_HAND				= 0x1000000,	// Required Offhand?
	FLAGS4_NO_PUSHBACK					= 0x2000000,    // Does not cause spell pushback
};
...
CHRONO_INLINE bool CanAgroHash(uint32 spellhashname)
{
    if (spellhashname == SPELL_HASH_HUNTER_S_MARK || spellhashname == SPELL_HASH_SAP || SPELL_HASH_EAGLE_EYE ) //hunter's mark
        return false;
    else
        return true;
}
...
/************************************************************************/
CHRONO_INLINE bool IsDamagingSpell(SpellEntry *sp)
{
    switch (sp->Effect[0])
    {
...
CHRONO_INLINE bool IsHealingSpell(SpellEntry *sp)
{
    switch( sp->Effect[0] )
    {
        case SPELL_EFFECT_HEALTH_LEECH:
...
CHRONO_INLINE bool IsInrange(LocationVector & location, Object * o, float square_r)
{
...
CHRONO_INLINE bool IsInrange(float x1,float y1, float z1,float x2,float y2, float z2,float square_r)
{
    float t;
    float r;
    t=x1-x2;
...
CHRONO_INLINE bool IsInrange(Object * o1,Object * o2,float square_r)
{
    return IsInrange(o1->GetPositionX(),o1->GetPositionY(),o1->GetPositionZ(),
        o2->GetPositionX(),o2->GetPositionY(),o2->GetPositionZ(),square_r);
}

CHRONO_INLINE bool TargetTypeCheck(Object *obj,uint32 ReqCreatureTypeMask)
{
...
CHRONO_INLINE bool IsTargetingStealthed(SpellEntry *sp)
{
	if(
...
public:
	...
	Spell* TriggerCallback;
	bool CanDelete;
	void EventDelete();

	int32 event_GetInstanceID();
	
    // Returns whether effect i hits hostile targets
    bool TargetsHostile(uint32 i);
    // Returns whether effect i hits friendly targets
    bool TargetsFriendly(uint32 i);
    ...
    CHRONO_INLINE uint32 getState() { return m_spellState; }
    CHRONO_INLINE void SetUnitTarget(Unit *punit){unitTarget=punit;}
	CHRONO_INLINE SpellEntry *GetSpellProto() { return m_spellInfo; }
	...
	CHRONO_INLINE uint32 GetType() { return ( m_spellInfo->Spell_Dmg_Type == SPELL_DMG_TYPE_NONE ? SPELL_DMG_TYPE_MAGIC : m_spellInfo->Spell_Dmg_Type ); }

    std::vector<uint64> UniqueTargets;
    SpellTargetsList    ModeratedTargets;

    CHRONO_INLINE Item* GetItemTarget() { return itemTarget; }
    CHRONO_INLINE Unit* GetUnitTarget() { /*return unitTarget;*/ return m_caster->GetMapMgr()->GetUnit(m_targets.m_unitTarget); }
    CHRONO_INLINE Player* GetPlayerTarget() { return playerTarget; }
    CHRONO_INLINE GameObject* GetGameObjectTarget() { return gameObjTarget; }
    ...
    CHRONO_INLINE float GetRadius(uint32 i)
    {
    ...
    CHRONO_INLINE static uint32 GetBaseThreat(uint32 dmg)
    {
...
/*        switch(m_spellInfo->NameHash)
	...
	CHRONO_INLINE static bool HasMechanic(SpellEntry * sp, uint32 MechanicsType)
	{
	...
	CHRONO_INLINE static uint32 GetMechanic(SpellEntry * sp)
	{
		if(sp->MechanicsType)
	...
	CHRONO_INLINE void safe_cancel()
	{
		m_cancelled = true;
	}
    ...
    /// Spell state's
    /// Spell failed
    CHRONO_INLINE bool GetSpellFailed(){return m_Spell_Failed;}
    CHRONO_INLINE void SetSpellFailed(bool failed = true){m_Spell_Failed = failed;}

    CHRONO_INLINE bool IsReflected() {return m_IsReflected;}
    CHRONO_INLINE void SetReflected(bool reflected = true) {m_IsReflected = reflected;}
    
    /// Spell possibility's
    CHRONO_INLINE bool GetCanReflect() {return m_CanRelect;}
    CHRONO_INLINE void SetCanReflect(bool reflect = true) {m_CanRelect = reflect;}

	//pet functions
	CHRONO_INLINE bool IsCorrectMobForTameQuest(uint32 unitId, uint32 spellid ) 
	{
			switch(spellid) 
            {       
...
protected:
	...
	CHRONO_INLINE bool HasTarget(const uint64& guid, TargetsList* tmpMap)
	{
