...
void WorldSession::HandleSwapItemOpcode(WorldPacket& recv_data)
{
	...
	//Item *SrcTemp = nullptr;
	//Item *DstTemp = nullptr;
	...
	{
		GetPlayer()->GetItemInterface()->BuildInventoryChangeError(SrcItem, nullptr, INV_ERR_CANT_DO_IN_COMBAT);
		return;
	}*/
	...
	else
	{
		...
		/*if (DstInvSlot != INVENTORY_SLOT_NOT_SET) 
		{
			...
			{
				_player->GetItemInterface()->BuildInventoryChangeError(SrcItem, nullptr, INV_ERR_ONLY_AMMO_CAN_GO_HERE);
				return;			
			}
		}*/
...
void WorldSession::HandleDestroyItemOpcode( WorldPacket & recv_data )
{
	...
	if(it)
	{
		...
		{
			_player->GetCurrentSpell()->i_caster=nullptr;
			_player->GetCurrentSpell()->cancel();
		}

		pItem->DeleteFromDB();
		//delete pItem; //-- Causes crash deleting the item from a secondary bag / character slot
	}
}
...
void WorldSession::HandleAutoEquipItemOpcode( WorldPacket & recv_data )
{
	...
	if(Slot == EQUIPMENT_SLOT_MAINHAND || Slot == EQUIPMENT_SLOT_OFFHAND)
	{
		...
		if( mainhandweapon != nullptr && mainhandweapon->GetProto()->InventoryType == INVTYPE_2HWEAPON )
		{
...
void WorldSession::HandleItemQuerySingleOpcode( WorldPacket & recv_data )
{
	...
	data << (itemProto->Class==ITEM_CLASS_CONSUMABLE ? uint32(0) : itemProto->SubClass);
	...
	for(i = 0; i < 5; i++) {
		...
		data << itemProto->Spells[i].Cooldown; //when not exist return -1
		data << itemProto->Spells[i].Category;
		data << itemProto->Spells[i].CategoryCooldown; //when not exist return -1
	}
...
void WorldSession::SendInventoryList(Creature* unit)
{
	...
	for(std::vector<CreatureItem>::iterator itr = unit->GetSellItemBegin(); itr != unit->GetSellItemEnd(); ++itr)
	{
		if(itr->itemid && (itr->max_amount == 0 || (itr->max_amount>0 && itr->available_amount >0)))
		{
			...
			{
				if(curItem->AllowableClass > -1 && !(_player->getClassMask() & curItem->AllowableClass))
					continue;
				if(curItem->AllowableRace > -1 && !(_player->getRaceMask() & curItem->AllowableRace))
					continue;
...
CHRONO_INLINE uint32 RepairItemCost(Player * pPlayer, Item * pItem)
{
...
CHRONO_INLINE void RepairItem(Player * pPlayer, Item * pItem)
{
...
void WorldSession::HandleRepairItemOpcode(WorldPacket &recvPacket)
{
	...
	else 
	{
		...
		if(item)
		{
			SlotResult *searchres=_player->GetItemInterface()->LastSearchResult();//this never gets nullptr since we get a pointer to the inteface internal var
...
void WorldSession::HandleBuyBankSlotOpcode(WorldPacket& recvPacket) 
{
	...
	price = (bsp != nullptr ) ? bsp->Price : 99999999;
