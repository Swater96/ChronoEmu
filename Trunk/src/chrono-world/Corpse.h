...
public:
   ...
   // void Create();
	...
	void SaveToDB();
	void DeleteFromDB();
	CHRONO_INLINE void SetCorpseState(uint32 state) { m_state = state; }
	CHRONO_INLINE uint32 GetCorpseState() { return m_state; }
	void Despawn();
	
	CHRONO_INLINE void SetLoadedFromDB(bool value) { _loadedfromdb = value; }
	CHRONO_INLINE bool GetLoadedFromDB(void) { return _loadedfromdb; }
	Loot loot;
	void generateLoot();
