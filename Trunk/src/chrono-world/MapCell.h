...
public:
	...
	CHRONO_INLINE size_t GetObjectCount() { return _objects.size(); }
	void RemoveObjects();
	CHRONO_INLINE ObjectSet::iterator Begin() { return _objects.begin(); }
	CHRONO_INLINE ObjectSet::iterator End() { return _objects.end(); }

	//State Related
	void SetActivity(bool state);

	CHRONO_INLINE bool IsActive() { return _active; }
	CHRONO_INLINE bool IsLoaded() { return _loaded; }

	//Object Loading Managing
	void LoadObjects(CellSpawns * sp);
	CHRONO_INLINE uint32 GetPlayerCount() { return _playerCount; }

	CHRONO_INLINE bool IsUnloadPending() { return _unloadpending; }
	CHRONO_INLINE void SetUnloadPending(bool up) { _unloadpending = up; }
