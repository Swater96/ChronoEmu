...
static int flagHonorTable[7]			= {  0,  5,  8, 14, 23, 38, 40};
static int winHonorTable[7]				= {  0,  2,  4,  7, 11, 19, 20};
// Not sure if we need these for 1.12
static int extraCompleteHonorTable[7]	= {  0,  7, 12, 20, 34, 57, 59}; // extras only for weekends
static int extraWinHonorTable[7]		= {  0,  5,  8, 14, 23, 38, 40};
...
WarsongGulch::WarsongGulch(MapMgr * mgr, uint32 id, uint32 lgroup, uint32 t) : CBattleground(mgr, id, lgroup, t)
{
	...
	if(m_dropFlags[1] == nullptr || !m_dropFlags[1]->CreateFromProto(179785, 489, 0, 0, 0, 0))
		Log.Warning("WarsongGulch", "Could not create dropped flag 1");

	m_dropFlags[0] = m_mapMgr->CreateGameObject(179786);
	if(m_dropFlags[0] == nullptr || !m_dropFlags[0]->CreateFromProto(179786, 489, 0, 0, 0, 0))
...
void WarsongGulch::HookOnAreaTrigger(Player * plr, uint32 id)
{
	...
	if(((id == 3646 && plr->GetTeam() == 0) || (id == 3647 && plr->GetTeam() == 1)) && (plr->m_bgHasFlag && m_flagHolders[plr->GetTeam()] == plr->GetLowGUID()))
	{
		...
		for(set<Player*>::iterator itr = m_players[plr->GetTeam()].begin(); itr != m_players[plr->GetTeam()].end(); ++itr)
		{
			...
			(*itr)->AddHonorCP(flagHonorTable[m_lgroup], HONORABLE, 0, 0);
		}
				...
				for(set<Player*>::iterator itr = m_players[i].begin(); itr != m_players[i].end(); ++itr)
				{
					...
					if(i == m_losingteam)
					{
						if (m_isWeekend)
						{
							...
							(*itr)->AddHonorCP(honor, HONORABLE, 0, 0);
						}
