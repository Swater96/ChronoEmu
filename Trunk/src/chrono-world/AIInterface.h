...
public:
	...
	CHRONO_INLINE Unit* getUnitToFollow() { return UnitToFollow; }
	void setCreatureState(CreatureState state){ m_creatureState = state; }
	CHRONO_INLINE uint8 getAIState() { return m_AIState; }
	CHRONO_INLINE uint8 getAIType() { return m_AIType; }
	CHRONO_INLINE uint8 getCurrentAgent() { return m_aiCurrentAgent; }
	...
	CHRONO_INLINE AssistTargetSet GetAssistTargets() { return m_assistTargets; }
	CHRONO_INLINE TargetMap *GetAITargets() { return &m_aiTargets; }
	...
	Unit* getTauntedBy();
	bool GetIsTaunted();
	CHRONO_INLINE size_t getAITargetsCount() { return m_aiTargets.size(); }
	CHRONO_INLINE uint32 getOutOfCombatRange() { return m_outOfCombatRange; }
	...
	CHRONO_INLINE bool getMoveRunFlag() { return m_moveRun; }
	CHRONO_INLINE void setCanMove(bool canmove) { m_canMove = canmove; }
	...
	CHRONO_INLINE bool GetAllowedToEnterCombat(void) { return m_AllowedToEnterCombat; }

	void CheckTarget(Unit* target);
	CHRONO_INLINE void SetAIState(AI_State newstate) { m_AIState = newstate; }
	...
	CHRONO_INLINE void AddStopTime(uint32 Time) { m_moveTimer += Time; }
	CHRONO_INLINE void SetNextSpell(AI_Spell * sp) { m_nextSpell = sp; }
	CHRONO_INLINE Unit* GetNextTarget() { return m_nextTarget; }
	CHRONO_INLINE void SetNextTarget (Unit *nextTarget) 
	{ 
	...
	CHRONO_INLINE void SetPetOwner(Unit * owner) { m_PetOwner = owner; }
	...
	CHRONO_INLINE void SetWaypointMap(WayPointMap * m) { m_waypoints = m; }
	bool m_isGuard;
	void CallGuards();
