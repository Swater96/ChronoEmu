...
void WorldSession::HandleTrainerListOpcode( WorldPacket & recv_data )
{
	...
	if(!train) return; //this was commented out altough i don't get why
...
void WorldSession::SendTrainerList(Creature* pCreature)
{
	...
	if(pTrainer == 0 || !CanTrainAt(_player, pTrainer)) return; // could break something
	...
	for(vector<TrainerSpell>::iterator itr = pTrainer->Spells.begin(); itr != pTrainer->Spells.end(); ++itr)
	{
		...
		data << pSpell->Cost;
		data << Spacer;
		data << Spacer; //is profesion but because the client doesn't notise a difference we can just leave it zero
...
void WorldSession::HandleTrainerBuySpellOpcode(WorldPacket& recvPacket)
{
	...
	data << (uint64) Guid;
	data << (uint32) TeachingSpellID;
	SendPacket(&data);
}

uint8 WorldSession::TrainerGetSpellStatus(TrainerSpell* pSpell)
{
	if( _player->HasSpell(pSpell->pLearnSpell->EffectTriggerSpell[0]) || _player->HasDeletedSpell(pSpell->pLearnSpell->EffectTriggerSpell[0]) )
		return TRAINER_STATUS_ALREADY_HAVE;

	if( pSpell->DeleteSpell && _player->HasDeletedSpell(pSpell->DeleteSpell) )
		return TRAINER_STATUS_ALREADY_HAVE;

	if( pSpell->pLearnSpell->Effect[1] == 44 && _player->_HasSkillLine(pSpell->pLearnSpell->EffectMiscValue[1]) == false && _player->_GetProfesionsAmount() > 1 )
	{
		skilllineentry* sk = dbcSkillLine.LookupEntry( pSpell->pLearnSpell->EffectMiscValue[1] );
		if( sk && sk->type != SKILL_TYPE_SECONDARY ) //we can always learn new secondary profesions :)
...
void WorldSession::SendInnkeeperBind(Creature* pCreature)
{
	...
	// Add a chrono if they don't have one
