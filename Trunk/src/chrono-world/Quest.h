...
enum QUEST_STATUS
{
	...
	QMGR_QUEST_AVAILABLELOW_LEVEL		= 0x01,	// Quest avaiable, but your level isnt enough.		| "Gray Quotation Mark !"
	...
	QMGR_QUEST_REWARD_REP				= 0x04, // Quest rewards REP								| "Blue Question ? Mark"
	QMGR_QUEST_AVAILABLE				= 0x05,	// Quest is available								 
	QMGR_QUEST_FINISHED_OLD				= 0x06,	// Quest avaiable, and your level is enough			| "Red Quotation ? Mark" 
	QMGR_QUEST_FINISHED					= 0x07,	// Quest has been finished.							| "Yellow Question  ? Mark" (7 has no minimap icon)
};
...
public:
	...
	CHRONO_INLINE Quest* GetQuest() { return m_quest; };
	...
	CHRONO_INLINE bool IsCastQuest() { return iscastquest;}
	...
	CHRONO_INLINE uint32 GetMobCount(uint32 i) { return m_mobcount[i]; }
	CHRONO_INLINE uint32 GetExploredAreas(uint32 i) { return m_explored_areas[i]; }

	CHRONO_INLINE uint32 GetBaseField(uint32 slot)
	{
