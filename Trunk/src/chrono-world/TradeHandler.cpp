...
void WorldSession::HandleSetTradeItem(WorldPacket & recv_data)
{
	...
	if (pTarget == nullptr || pItem == nullptr || TradeSlot > 6)
		return;
	if (TradeSlot < 6)
	{
	...
	if (SourceSlot >= INVENTORY_SLOT_BAG_START && SourceSlot < INVENTORY_SLOT_BAG_END)
	{
		...
		if (itm == nullptr || SourceSlot >= itm->GetProto()->ContainerSlots) //Required as there are bags with SourceSlot > INVENTORY_SLOT_BAG_START
		{
			//More duping woohoo
			sCheatLog.writefromsession(this, "tried to cheat trade a soulbound item");
			Disconnect();
