...
public:
	...
	CHRONO_INLINE ItemPrototype* GetProto() const { return m_itemProto; }
	CHRONO_INLINE void SetProto( ItemPrototype* pr ) { m_itemProto = pr; }

	CHRONO_INLINE Player* GetOwner() const { return m_owner; }
	void SetOwner( Player* owner );

	CHRONO_INLINE bool IsContainer(){ return ( m_objectTypeId == TYPEID_CONTAINER ) ? true : false; }
	...
	CHRONO_INLINE time_t GetEnchantmentApplytime( uint32 slot )
	{
	...
	CHRONO_INLINE void SetCount( uint32 amt ) { SetUInt32Value( ITEM_FIELD_STACK_COUNT, amt ); }
	CHRONO_INLINE void SetDurability( uint32 Value ) { SetUInt32Value(ITEM_FIELD_DURABILITY, Value ); };
	CHRONO_INLINE void SetDurabilityToMax() { SetUInt32Value( ITEM_FIELD_DURABILITY, GetUInt32Value( ITEM_FIELD_MAXDURABILITY ) ); }
	CHRONO_INLINE uint32 GetDurability() { return GetUInt32Value( ITEM_FIELD_DURABILITY ); }
	CHRONO_INLINE uint32 GetDurabilityMax() { return GetUInt32Value( ITEM_FIELD_MAXDURABILITY ); }
	CHRONO_INLINE bool IsAmmoBag() { return (m_itemProto->Class == ITEM_CLASS_QUIVER); }
	...
	CHRONO_INLINE uint32 GetItemRandomSuffixFactor() { return m_uint32Values[ITEM_FIELD_PROPERTY_SEED]; }

	static uint32 GenerateRandomSuffixFactor( ItemPrototype* m_itemProto );

	CHRONO_INLINE void SetRandomProperty( uint32 id )
	{
		SetUInt32Value( ITEM_FIELD_RANDOM_PROPERTIES_ID, id );
		random_prop = id;
	}

	CHRONO_INLINE void SetRandomSuffix( uint32 id )
	{
		int32 r_id = -(int32(id));
