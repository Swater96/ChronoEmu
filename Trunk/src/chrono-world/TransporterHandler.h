...
public:
	...
	CHRONO_INLINE void SetLength(const unsigned int sz)
	{
		i_nodes.resize( sz );
	}

	CHRONO_INLINE size_t Size(void) const { return i_nodes.size(); }
	CHRONO_INLINE void Resize(unsigned int sz) { i_nodes.resize(sz); }
	CHRONO_INLINE void Clear(void) { i_nodes.clear(); }
	CHRONO_INLINE PathNode* GetNodes(void) { return static_cast<PathNode *>(&i_nodes[0]); }
...
public:
	...
	CHRONO_INLINE void AddPlayer(Player *pPlayer) { mPassengers[pPlayer->GetLowGUID()] = pPlayer; }
	CHRONO_INLINE void RemovePlayer(Player *pPlayer) {mPassengers.erase(pPlayer->GetLowGUID()); }
	CHRONO_INLINE bool HasPlayer(Player* pPlayer) { return mPassengers.find(pPlayer->GetLowGUID()) != mPassengers.end(); }
	CHRONO_INLINE void SetPeriod(uint32 val) { m_period = val; }
