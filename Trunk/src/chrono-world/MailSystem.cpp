...
bool MailMessage::AddMessageDataToPacket(WorldPacket& data)
{
	...
	data << (uint32) message_id;
	data << (uint32) 0;
	data << (uint32) stationary;

	pItem = objmgr.LoadItem( item_guid );

	data << (uint32) (pItem ? pItem->GetEntry() : 0);
	//data << (uint32) (pItem ? pItem->GetEnchantmentCharges((EnchantmentSlot)PERM_ENCHANTMENT_SLOT) : 0);
	data << (uint32) 0;
	data << (uint32) (pItem ? pItem->GetUInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID) : 0);
	data << (uint32) 0;
	//data << (uint32) (pItem ? pItem->GetItemRandomSuffixFactor() : 0);
	data << (uint8)  (pItem ? pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT) : 0);
	data << (uint32) (pItem ? pItem->GetChargesLeft() : 0);
	data << (uint32) (pItem ? pItem->GetUInt32Value(ITEM_FIELD_MAXDURABILITY) : 0);
	data << (uint32) (pItem ? pItem->GetUInt32Value(ITEM_FIELD_DURABILITY) : 0);
...
void WorldSession::HandleSendMail(WorldPacket & recv_data )
{
	...
	/* not sure about this part so comment out to be safe -BT
	
	if(msg.money != 0 || msg.cod != 0 || msg.item_guid != 0 && player->acct != _player->GetSession()->GetAccountId()) 
	{
...
void WorldSession::HandleTakeItem(WorldPacket & recv_data )
{
	...
	MailMessage * message = _player->m_mailBox.GetMessage(message_id);
	if(message == 0 || message->item_guid == 0)
	{
		data << uint32(MAIL_ERR_INTERNAL_ERROR);
		SendPacket(&data);
	...
	// grab the item
	Item * item = objmgr.LoadItem( message->item_guid );
	if(item == 0)
	{
		// doesn't exist
		data << uint32(MAIL_ERR_INTERNAL_ERROR);
...
void WorldSession::HandleReturnToSender(WorldPacket & recv_data )
{
	...
	message.delivery_time = message.item_guid == 0 ? (uint32)UNIXTIME : (uint32)UNIXTIME + 3600;
...
void Mailbox::Load(QueryResult * result)
{
	...
	do 
	{
		...
		msg.item_guid = fields[i++].GetUInt64();
