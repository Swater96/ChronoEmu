...
public:
	...
	CHRONO_INLINE uint32 GetBlockCount() const {return mBlocks;}
	
	CHRONO_INLINE uint32 GetLength() const { return (mBlocks *sizeof(uint32)); }
	CHRONO_INLINE uint32 GetCount() const { return mCount; }
	CHRONO_INLINE const uint8* GetMask() const { return (uint8*)mUpdateMask; }
