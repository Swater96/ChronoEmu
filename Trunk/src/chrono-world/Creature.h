...
public:
	...
	CHRONO_INLINE void GetSellItemBySlot(uint32 slot, CreatureItem &ci)
	{
	...
	CHRONO_INLINE std::vector<CreatureItem>::iterator GetSellItemBegin() { return m_SellItems->begin(); }
	CHRONO_INLINE std::vector<CreatureItem>::iterator GetSellItemEnd()   { return m_SellItems->end(); }
	CHRONO_INLINE size_t GetSellItemCount() { return m_SellItems->size(); }
	...
	CHRONO_INLINE void SetSheatheForAttackType(uint8 type) { SetByte(UNIT_FIELD_BYTES_2, 0, type); }
	CHRONO_INLINE bool isQuestGiver() { return HasFlag( UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER ); };
	...
	CHRONO_INLINE uint32 GetSQL_id() { return spawnid; };
	...
	CHRONO_INLINE Player *GetTotemOwner() { return totemOwner; }
	CHRONO_INLINE void SetTotemOwner(Player *owner) { totemOwner = owner; }
	CHRONO_INLINE uint32 GetTotemSlot() { return totemSlot; }
	CHRONO_INLINE void SetTotemSlot(uint32 slot) { totemSlot = slot; }

	CHRONO_INLINE bool IsPickPocketed() { return m_PickPocketed; }
	CHRONO_INLINE void SetPickPocketed(bool val = true) { m_PickPocketed = val; }

	CHRONO_INLINE CreatureAIScript * GetScript() { return _myScriptClass; }
	void LoadScript();
	...
	CHRONO_INLINE CreatureInfo *GetCreatureInfo()
	{
		return creature_info; 
	}
	// left this function for backwards compatibility with scripts
	// please use GetCreatureInfo()
	CHRONO_INLINE CreatureInfo *GetCreatureName()
	{
		return creature_info; 
	}
	CHRONO_INLINE void SetCreatureName(CreatureInfo *ci) { creature_info = ci; }
	CHRONO_INLINE Trainer* GetTrainer() { return mTrainer; }
	void RegenerateFocus();

	CreatureFamilyEntry * myFamily;
	CHRONO_INLINE bool IsTotem() { return totemOwner != 0 && totemSlot != -1; }
	void TotemExpire();
	...
	void SetCreatureEntry(uint32 entry) 
	{ 
...
public:
	...
	bool ChangeEntry(uint32 Entry);
	CHRONO_INLINE CreatureProto *GetProto() { return proto; }
