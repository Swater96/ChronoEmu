...
/*	typedef struct { const char *name; int(*mfunc)(lua_State*,GameObject*); } RegType;
	...
	CHRONO_INLINE GameObjectInfo* GetInfo() { return pInfo; }
	CHRONO_INLINE void SetInfo(GameObjectInfo * goi) { pInfo = goi; }
   ...
   // void Create ( uint32 mapid, float x, float y, float z, float ang);
	...
	CHRONO_INLINE bool isQuestGiver()
	{
	...
	CHRONO_INLINE GameObjectAIScript* GetScript() { return myScript; }

	void TrapSearchTarget();	// Traps need to find targets faster :P

	CHRONO_INLINE bool HasAI() { return spell != 0; }
	GOSpawn * m_spawn;
	void OnPushToWorld();
	void OnRemoveInRangeObject(Object* pObj);
	void RemoveFromWorld(bool free_guid);

	CHRONO_INLINE bool CanMine(){return (mines_remaining > 0);}
	CHRONO_INLINE void UseMine(){ if(mines_remaining) mines_remaining--;}
