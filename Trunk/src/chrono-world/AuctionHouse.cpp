...
{
	CharacterDatabase.Execute("INSERT INTO auctions VALUES(%u, %u, " I64FMT ", %u, %u, %u, %u, %u, %u)",
...
void Auction::AddToPacket(WorldPacket & data)
{
    data << Id;
    data << pItem->GetEntry();
    data << (uint32) pItem->GetUInt32Value(ITEM_FIELD_ENCHANTMENT+0*3+0);    // Enchantment ID
    data << (uint32) pItem->GetUInt32Value(ITEM_FIELD_RANDOM_PROPERTIES_ID); // -ItemRandomSuffix / random property     : If the value is negative its ItemRandomSuffix if its possitive its RandomItemProperty
    data << uint32(0);			
	data << pItem->GetUInt32Value(ITEM_FIELD_STACK_COUNT); // Amount
	data << pItem->GetUInt32Value(ITEM_FIELD_SPELL_CHARGES);
	data << uint32(Owner);			  // Owner guid
    data << uint32(0);   
	...
	data << uint32(HighestBidder);	  // Last bidder
	data << uint32(0);
