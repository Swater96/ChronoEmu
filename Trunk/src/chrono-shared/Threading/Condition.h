...
protected:
	HANDLE create_anonymous_semaphore(long cur,long max)
	{
		...
		CHRONO_INLINE void shift_generations_down()
		{
		...
		CHRONO_INLINE void dispose_entry(list_entry& entry)
		{
		...
		CHRONO_INLINE HANDLE duplicate_handle(HANDLE source)
		{
...
{
public:
	CHRONO_INLINE Condition(Mutex * mutex) : m_nLockCount(0), m_externalMutex(mutex)
	{
	...
	CHRONO_INLINE void BeginSynchronized()
	{
		m_externalMutex->Acquire();
		++m_nLockCount;
	}

	CHRONO_INLINE void EndSynchronized()
	{
...
public:
	...
	CHRONO_INLINE void Signal()
	{
		pthread_cond_signal(&cond);
	}
	CHRONO_INLINE void Broadcast()
	{
		pthread_cond_broadcast(&cond);
	}
	CHRONO_INLINE void Wait()
	{
		pthread_cond_wait(&cond,&mut->mutex);
	}
	CHRONO_INLINE bool Wait(time_t seconds)
	{
		timespec tv;
		tv.tv_nsec = 0;
		tv.tv_sec = seconds;
