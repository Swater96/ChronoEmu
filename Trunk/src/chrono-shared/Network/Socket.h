...
/* Sending Operations */
	...
	CHRONO_INLINE void BurstBegin() { m_writeMutex.Acquire(); }
	...
	CHRONO_INLINE void BurstEnd() { m_writeMutex.Release(); }

/* Client Operations */

	// Get the client's ip in numerical form.
	string GetRemoteIP();
	CHRONO_INLINE uint32 GetRemotePort() { return ntohs(m_client.sin_port); }
	CHRONO_INLINE SOCKET GetFd() { return m_fd; }
...
/* Platform-specific methods */
	...
	CHRONO_INLINE bool IsDeleted() { return m_deleted; }
	CHRONO_INLINE bool IsConnected() { return m_connected; }
	CHRONO_INLINE sockaddr_in & GetRemoteStruct() { return m_client; }
	CHRONO_INLINE CircularBuffer& GetReadBuffer() { return readBuffer; }
	CHRONO_INLINE CircularBuffer& GetWriteBuffer() { return writeBuffer; }

/* Deletion */
	void Delete();

	CHRONO_INLINE in_addr GetRemoteAddress() { return m_client.sin_addr; }
...
public:
	...
	CHRONO_INLINE void SetCompletionPort(HANDLE cp) { m_completionPort = cp; }
	
	// Atomic wrapper functions for increasing read/write locks
	CHRONO_INLINE void IncSendLock() { InterlockedIncrement(&m_writeLock); }
	CHRONO_INLINE void DecSendLock() { InterlockedDecrement(&m_writeLock); }
	CHRONO_INLINE bool AcquireSendLock()
	{
		if(m_writeLock)
			return false;
		else
...
public:
	...
	// Atomic wrapper functions for increasing read/write locks
	CHRONO_INLINE void IncSendLock() { __sync_add_and_fetch(&m_writeLock, 1); }
	CHRONO_INLINE void DecSendLock() { __sync_sub_and_fetch(&m_writeLock, 1); }
	CHRONO_INLINE bool HasSendLock() { return (m_writeLock != 0); }
