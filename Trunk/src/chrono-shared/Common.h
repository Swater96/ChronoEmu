...
#ifdef WIN32
#define CHRONO_INLINE __forceinline
#else
#define CHRONO_INLINE inline
#endif
...
CHRONO_INLINE void reverse_array(uint8 * pointer, size_t count)
{
	size_t x;
...
CHRONO_INLINE void ASCENT_TOLOWER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
		str[i] = (char)tolower(str[i]);
};

CHRONO_INLINE void ASCENT_TOUPPER(std::string& str)
{
	for(size_t i = 0; i < str.length(); ++i)
