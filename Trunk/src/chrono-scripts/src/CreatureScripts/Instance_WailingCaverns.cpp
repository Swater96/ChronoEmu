...
//Deviate Adder
#define CN_DEVIATE_ADDER		5048
class DeviateAdderAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateAdderAI, MoonScriptCreatureAI);		
    DeviateAdderAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Poison
		AddSpell(744, Target_Current, 30, 0, 30);	
	}
};

//Deviate Crockolisk
#define CN_DEVIATE_CROCKOLISK		5053
class DeviateCrockoliskAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateCrockoliskAI, MoonScriptCreatureAI);		
    DeviateCrockoliskAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Tendon rip
		AddSpell(3604, Target_Current, 30, 0, 8);	
	}
};

//Deviate Dreadfang
#define CN_DEVIATE_DREADFANG		5056
class DeviateDreadfangAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateDreadfangAI, MoonScriptCreatureAI);		
    DeviateDreadfangAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Terrify
		AddSpell(7399, Target_Current, 30, 0, 10);	
	}
};

//Deviate Lasher
#define CN_DEVIATE_LASHER		5055
class DeviateLasherAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateLasherAI, MoonScriptCreatureAI);		
    DeviateLasherAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Wide slash
		AddSpell(7342, Target_Current, 30, 0, 10);	
	}
};
...
//Deviate Shambler
#define CN_DEVIATE_SHAMBLER		5761
class DeviateShamblerAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateShamblerAI, MoonScriptCreatureAI);		
    DeviateShamblerAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Wild regeneration
		AddSpell(7948, Target_Self, 30, 3, 15);	
	}
};

//Deviate Venomwing
#define CN_DEVIATE_VENOMWING		5756
class DeviateWenomwingAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateWenomwingAI, MoonScriptCreatureAI);		
    DeviateWenomwingAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Toxic spit
		AddSpell(7951, Target_Current, 30, 2.5, 5);	
	}
};

//Deviate Viper
#define CN_DEVIATE_VIPER		5755
class DeviateViperAI : public MoonScriptCreatureAI
{
	MOONSCRIPT_FACTORY_FUNCTION(DeviateViperAI, MoonScriptCreatureAI);		
    DeviateViperAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		//Localized Toxin
		AddSpell(7947, Target_Current, 10, 0, 30);	
	}
};
...
class DruidFangAI : public MoonScriptCreatureAI
{
    ...
    void AIUpdate()
    {
		if(SerpentForm->mEnabled && GetHealthPercent() <= 50)
		{
		...
		if(HealingTouch->mEnabled && GetHealthPercent() <= 5)
		{
			// Remove Serpent Form
...
class LadyAnacondraAI : public MoonScriptCreatureAI
{
    ...
    LadyAnacondraAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		...
		ApplyAura(8148);
		// Healing touch
		AddSpell(5187, Target_Self, 20, 2.5, 20);
	}
};
...
public:
    ...
    LordCobrahnAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		...
		LightningBolt = AddSpell(9532, Target_Current, 30, 3, 0);
		// Slumber
		Slumber = AddSpell(8040, Target_RandomPlayerNotCurrent, 10, 2.5, 20);
		// Poison
		Poison = AddSpell(744, Target_Current, 15, 0, 10);
		Poison->mEnabled = false;
		// Healing touch
		Heal = AddSpell(5188, Target_Self, 20, 3, 20);
    ...
    void AIUpdate()
    {
		if(SerpentForm->mEnabled && GetHealthPercent() <= 20)
		{
			SerpentForm->mEnabled = false;
			CastSpellNowNoScheduling(SerpentForm);
			LightningBolt->mEnabled = false;
			Slumber->mEnabled = false;
			Heal->mEnabled = false;
			Poison->mEnabled = true;
		}
		ParentClass::AIUpdate();
    }
};
...
class LordSerpentisAI : public MoonScriptCreatureAI
{
    ...
    LordSerpentisAI(Creature * pCreature) : MoonScriptCreatureAI(pCreature)
    {
		...
		AddSpell(700, Target_RandomPlayerNotCurrent, 10, 1.5, 20);
		// Healing touch
		AddSpell(6778, Target_Self, 30, 3, 20);
	}
};
...
class VerdanEverlivingAI : public MoonScriptCreatureAI
{
    ...
    {
		// Grasping Vines
		AddSpell(8142, Target_Self, 30, 1, 10);		
	}
};
...
class MutanusAI : public MoonScriptCreatureAI
{
    ...
    {
		// Naralex's nightmare
		AddSpell(7967, Target_RandomPlayerNotCurrent, 15, 2, 10);
		// Thundercrack
		AddSpell(8150, Target_Self, 15, 0, 0);
		// Terrify
		AddSpell(7399, Target_RandomPlayer, 15, 0, 10);		
	}
};
...
{
	mgr->register_creature_script(CN_DEVIATE_ADDER, &DeviateAdderAI::Create);
	mgr->register_creature_script(CN_DEVIATE_CROCKOLISK, &DeviateCrockoliskAI::Create);
	mgr->register_creature_script(CN_DEVIATE_DREADFANG, &DeviateDreadfangAI::Create);
	mgr->register_creature_script(CN_DEVIATE_LASHER, &DeviateLasherAI::Create);
	mgr->register_creature_script(5762, &DeviateMoccasinAI::Create);
	mgr->register_creature_script(CN_DEVIATE_SHAMBLER, &DeviateShamblerAI::Create);
	mgr->register_creature_script(CN_DEVIATE_VENOMWING, &DeviateWenomwingAI::Create);
	mgr->register_creature_script(CN_DEVIATE_VIPER, &DeviateViperAI::Create);
	...
	mgr->register_gossip_script(CN_DIS_NARALEX, (GossipScript*) new DofNaralexGossip());
